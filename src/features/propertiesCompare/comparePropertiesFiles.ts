...
function getMalformedIssues(parsed: ParsedEnvFile): LineIssue[] {
  return parsed.issues
    .filter((issue) => issue.code === 'malformed_line')
...
export function comparePropertiesFiles(
  ...
  const templateKeys = new Set(templateEffectiveEntries.map((entry) => entry.normalizedKey));
  const requiredTemplateKeys = templateEffectiveEntries;
  ...
  const environments: EnvironmentCompareResult[] = environmentFiles.map((environment) => {
    const envKeys = new Set(environment.parsed.effectiveEntries.map((entry) => entry.normalizedKey));
  ...
  return {
    template: {
      requiredKeys: requiredTemplateKeys.map((entry) => ({
        ...
        hasDefaultValue: entry.value.trim().length > 0,
      })),
